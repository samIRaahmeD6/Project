...
<body>
     <div class="wrapper1">
      ...
      <form action="member_logcc.php" method='POST' autocomplete="off">
          <div class="label3">
...
if(!empty($uni_id) && !empty($password)){
  $sql1 = "SELECT * FROM member_reg WHERE uni_id = '$uni_id' AND password = '$password' AND club_name ='computer club' AND status ='1'";
  $sql2 = "SELECT * FROM member_reg WHERE uni_id = '$uni_id' AND password = '$password' AND club_name ='computer club' AND status ='0'";
  $query1 = $conn->query($sql1);
  $query2 = $conn->query($sql2);
 
  if($query1->num_rows > 0){
    $row = mysqli_fetch_array($query1);
    header('location:computerclub.php');
  }
